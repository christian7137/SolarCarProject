...
// initialize  CAN
	...
	svtSensor* luminosity = new Luminositysensor();
	Thread::wait(400);
//	gblinking->setCANBASE(CANaddr + (GreenLedSensorID << 4));
//	gblinking->attachCAN(canbus);   // attach sensor to CAN bus
	//luminosity->setPeriod(250);       // set update period (ms)
        luminosity->attachClock(&msclock);

	// initialize sensor and add to sensor Manager
	if(!(luminosity->init())){
	    smanager.addSensor(luminosity);
	}
	else{  // process initialization error - report failure
	    udebug.printf("Luminosity failed to initialize.\r\n");
	}
#endif 
