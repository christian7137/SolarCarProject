...
int main(int argc, char *argv[])
{
    ...
    int count = 0;      //testing
	server.possError = sendto(server.sock,(struct Json_Message*)&server.json_message, strlen(buffer),0,(const struct sockaddr *)&server.serverAddr,server.length);//change strlen(buffer)
    /*while(count < 100){
       server.sendPacket(buffer);
       //code below is testing
       count++;
